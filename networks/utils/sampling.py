...
def generate_real_images(image_processor, n_samples, shape, transform_type="old_to_new"):
    x_real = image_processor.sample_tensor(n_samples)
    if transform_type is not None:
        x_real = image_processor.transform_tensor(tensor=x_real, transform_type=transform_type)
    if x_real.shape[1] != shape[0]:
        x_real = image_processor.resize_tensor(tensor=x_real, shape=shape)
    return x_real
...
def generate_fake_images(image_processor, generator, n_samples, shape, transform_type=None):
    ...
    if transform_type is not None:
        x_fake = image_processor.transform_tensor(tensor=x_fake, transform_type=transform_type)
    if x_fake.shape[1] != shape[0]:
        x_fake = image_processor.resize_tensor(tensor=x_fake, shape=shape)
    return x_fake


def generate_fake_images_from_latents(z_latent, image_processor, generator, shape, transform_type=None):
    x_fake = generator(z_latent, training=False)
    if transform_type is not None:
        x_fake = image_processor.transform_tensor(tensor=x_fake, transform_type=transform_type)
    if x_fake.shape[1] != shape[0]:
        x_fake = image_processor.resize_tensor(tensor=x_fake, shape=shape)
